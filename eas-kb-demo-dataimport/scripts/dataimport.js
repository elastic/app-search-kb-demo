...
const PROGRESS_BAR_CONFIG = {
  ...
  hideCursor: true,
  format: '  Importing {filename} [{bar}] {percentage}% | ETA: {eta}s | {value}/{total}'
};
...
const importFile = (client, filename, engine, progressBar) => {
  return client.getEngine(engine)
    ...
    .then(async () => {
      ...
      fileProgressBar.update(0, {filename: filename.padEnd(80)});
      for (var i = 0; i < jsonData.length; i += BATCH_SIZE) {
        var docs = jsonData.slice(i, i + BATCH_SIZE).map(processDoc);

        await client.indexDocuments(engine, docs);
